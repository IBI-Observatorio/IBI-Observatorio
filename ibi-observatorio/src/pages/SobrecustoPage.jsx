...
// Ferrogrão (single load — soja + milho)
const FG_BASE = {
  vol_2026: 26.0,
  vol_2030: 33.5,
  frete_pico: 300,
...
function computeFerrograo(p) {
  ...
  const calc = (vol, dif) => vol * 1e6 * dif * p.captura;

  return {
    fretePondBase, fretePondEstr, custoFerrBase, custoFerrPess,
    difBase, difEstr, difPess,
...
// EF-118 (segmented by cargo type)
const EF_BASE = {
  vol_2026: 10.0,
  vol_2040: 20.0,
  vol_2050: 25.0,
...
const EF_SEGMENTS = [
  // [nome, %mix, dist_rod, tarifa_rod, dist_ferr, tarifa_ferr, perna]
  ['Granéis agrícolas + fertilizantes', 0.28, 1500, 0.150, 1000, 0.070, 300],
  ['Carga conteinerizada (SP-RJ-ES)',   0.12,  500, 0.180,  575, 0.080,  50],
  ['Carvão e coque (importação)',       0.10,  550, 0.170,  550, 0.060,  50],
  ['Combustíveis (granéis líquidos)',   0.08,  400, 0.200,  400, 0.070,  50],
  ['Minério de ferro (já via EFVM)',    0.05,    0, 0.000,    0, 0.000,   0],
  ['Ferro gusa (MG → portos)',          0.02,  600, 0.170,  600, 0.060,  80],
  ['HBI (porto-indústria Açu)',         0.33,    5, 0.300,    5, 0.000,   0],
  ['Outros (rocha, calcário)',          0.02,  400, 0.180,  400, 0.070,  80],
];

function computeEF118(p) {
  const segments = EF_SEGMENTS.map(([nome, pct, drod, trod, dferr, tferr, perna]) => {
  ...
  const calcYear = (vol) => {
    const sob = (cenário) => segments.reduce((acc, s) =>
      acc + vol * s.pct * 1e6 * s[cenário] * p.captura, 0);
...
const fmtBRLBi = (v) => `R$ ${(v/1e9).toLocaleString('pt-BR', { maximumFractionDigits: 2 })} bi`;
const fmtBRLMi = (v) => `R$ ${(v/1e6).toLocaleString('pt-BR', { maximumFractionDigits: 0 })} mi`;
const fmtNum = (v, dec = 1) => v.toLocaleString('pt-BR', { minimumFractionDigits: dec, maximumFractionDigits: dec });
const fmtPct = (v) => `${(v*100).toFixed(0)}%`;

function smartMoney(v) {
...
const Eyebrow = ({ children, color = C.green }) => (
  <div style={{
    fontFamily: fontStack, fontSize: 11, fontWeight: 700, letterSpacing: '0.18em',
    textTransform: 'uppercase', color, marginBottom: 16
  }}>{children}</div>
);

const Hr = ({ color = C.rule, my = 24 }) => (
  <div style={{ height: 1, background: color, margin: `${my}px 0` }} />
);

const Number = ({ children, size = 'md', color = C.navy }) => {
  const sizes = { sm: 18, md: 28, lg: 44, xl: 64, xxl: 88 };
  return (
    <span style={{
      fontFamily: fontStack, fontWeight: 700, color,
      fontSize: sizes[size], lineHeight: 1, fontVariantNumeric: 'tabular-nums',
      letterSpacing: '-0.02em',
    }}>{children}</span>
  );
};

const KeyMetric = ({ label, value, sublabel, accent }) => (
  <div style={{
    background: C.paper, border: `1px solid ${C.rule}`, padding: '24px 22px',
    borderTop: accent ? `3px solid ${accent}` : `1px solid ${C.rule}`,
    borderRadius: 2, height: '100%',
  }}>
    <div style={{
      fontFamily: fontStack, fontSize: 10, fontWeight: 600, letterSpacing: '0.12em',
      textTransform: 'uppercase', color: C.muted, marginBottom: 12
    }}>{label}</div>
    <div style={{ marginBottom: sublabel ? 6 : 0 }}>
      <Number size="lg">{value}</Number>
    </div>
    {sublabel && (
      <div style={{ fontFamily: fontStack, fontSize: 12, color: C.muted, lineHeight: 1.4 }}>
        {sublabel}
      </div>
    )}
  </div>
  ...
  return (
    <div style={{ marginBottom: 28 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 10 }}>
        <div>
          <div style={{ fontFamily: fontStack, fontSize: 13, fontWeight: 600, color: C.text, letterSpacing: '0.01em' }}>
            {label}
          </div>
          {hint && (
            <div style={{ fontFamily: fontStack, fontSize: 11, color: C.muted, marginTop: 3 }}>{hint}</div>
          )}
        </div>
        <div style={{
          fontFamily: fontStack, fontWeight: 700, fontSize: 16, color: C.navy,
      ...
      <input
        ...
        style={{
          width: '100%', height: 4, borderRadius: 0, appearance: 'none',
          background: `linear-gradient(to right, ${C.navy} 0%, ${C.navy} ${((value-min)/(max-min))*100}%, ${C.rule} ${((value-min)/(max-min))*100}%, ${C.rule} 100%)`,
...
function ScenarioChart({ data, height = 280 }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={chartData} margin={{ top: 24, right: 16, left: 0, bottom: 8 }}>
        ...
        <XAxis dataKey="cenário" tick={{ fill: C.muted, fontSize: 12, fontFamily: fontStack }}
               axisLine={{ stroke: C.rule }} tickLine={false} />
        <YAxis tick={{ fill: C.muted, fontSize: 11, fontFamily: fontStack }}
               axisLine={false} tickLine={false}
               tickFormatter={(v) => `R$ ${v.toFixed(1)} bi`} />
        <Tooltip
          contentStyle={{ background: C.paper, border: `1px solid ${C.rule}`, borderRadius: 2, fontFamily: fontStack, fontSize: 12 }}
          formatter={(v) => [`R$ ${v.toFixed(2)} bi/ano`, '']}
...
function SegmentChart({ segments, totalVol, captura }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <BarChart data={data} layout="vertical" margin={{ top: 8, right: 16, left: 0, bottom: 8 }}>
        ...
        <XAxis type="number" tick={{ fill: C.muted, fontSize: 11, fontFamily: fontStack }}
               axisLine={false} tickLine={false}
               tickFormatter={(v) => `R$ ${v.toFixed(0)} mi`} />
        <YAxis type="category" dataKey="nome" tick={{ fill: C.text, fontSize: 11, fontFamily: fontStack }}
               axisLine={{ stroke: C.rule }} tickLine={false} width={170} />
        <Tooltip
          contentStyle={{ background: C.paper, border: `1px solid ${C.rule}`, borderRadius: 2, fontFamily: fontStack, fontSize: 12 }}
          formatter={(v) => [`R$ ${v.toFixed(0)} mi/ano`, 'Sobrecusto']}
          labelStyle={{ color: C.navy, fontWeight: 700 }}
        />
        ...
        <tbody>
          {rows.map((r, i) => (
            r.section ? (
              <tr key={i}>
                <td colSpan={3} style={{
                  padding: '14px 18px 6px', background: C.paper,
                  fontSize: 10, fontWeight: 700, letterSpacing: '0.12em',
                  textTransform: 'uppercase', color: C.navy, borderTop: i ? `1px solid ${C.rule}` : 'none'
                }}>{r.section}</td>
              </tr>
            ) : (
  ...
  return (
    <div style={{ borderTop: `1px solid ${C.rule}`, borderBottom: `1px solid ${C.rule}` }}>
      <button onClick={() => setOpen(!open)} style={{
        ...
        cursor: 'pointer', fontFamily: fontStack, fontSize: 13, fontWeight: 600,
...
// =====================================================================
// HERO COUNTER (sobrecusto agregado, em tempo real)
// =====================================================================

function LiveCounter({ annualTotal }) {
  ...
  return (
    <div>
      <div style={{
        fontFamily: fontStack, fontWeight: 700, color: C.cream,
        fontSize: 'clamp(48px, 9vw, 128px)', lineHeight: 0.9,
      ...
      <div style={{ marginTop: 18, display: 'flex', gap: 32, alignItems: 'baseline', flexWrap: 'wrap' }}>
        <div>
          <div style={{ fontFamily: fontStack, fontSize: 11, color: C.green, letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 700, marginBottom: 4 }}>
...
// =====================================================================
// CORREDOR SECTION (reusable)
// =====================================================================

function CorredorFerrograo() {
  const [params, setParams] = useState({ ...FG_BASE });
  ...
  return (
    <CorredorBlock
      ...
      sliders={
        <>
          <Slider label="Volume capturável 2026"
                  value={params.vol_2026} min={10} max={50} step={0.5}
                  onChange={set('vol_2026')}
                  format={(v) => `${fmtNum(v)} Mt`}
                  hint="Toda a carga MT → Arco Norte que poderia migrar" />
          <Slider label="Frete rodoviário em pico de safra"
                  value={params.frete_pico} min={200} max={400} step={10}
                  onChange={set('frete_pico')}
                  format={(v) => fmtBRL(v) + '/t'}
                  hint="Soja Fev-Abr + milho Jul-Set (60% do volume)" />
          <Slider label="Captura modal efetiva"
                  value={params.captura} min={0.4} max={1.0} step={0.05}
                  onChange={set('captura')}
                  format={fmtPct}
                  hint="% da carga que efetivamente migra para a ferrovia" />
          <Slider label="Perna rodoviária residual"
                  value={params.perna_km} min={50} max={300} step={10}
                  onChange={set('perna_km')}
                  format={(v) => `${v} km`}
                  hint="Distância fazenda → terminal ferroviário" />
        </>
      }
...
function CorredorEF118() {
  const [params, setParams] = useState({ ...EF_BASE });
  ...
  return (
    <CorredorBlock
      ...
      sliders={
        <>
          <Slider label="Volume capturável 2026"
                  value={params.vol_2026} min={5} max={25} step={0.5}
                  onChange={set('vol_2026')}
                  format={(v) => `${fmtNum(v)} Mt`}
                  hint="Inclui agrícola, contêiner, carvão, HBI" />
          <Slider label="Captura modal efetiva"
                  value={params.captura} min={0.4} max={1.0} step={0.05}
                  onChange={set('captura')}
                  format={fmtPct}
                  hint="% do mix que efetivamente migra" />
          <Slider label="Multiplicador do frete rodoviário"
                  value={params.mult_estr} min={1.0} max={1.5} step={0.05}
                  onChange={set('mult_estr')}
                  format={(v) => `${fmtNum(v, 2)}x`}
                  hint="Cenário estressado: gargalo BR-101 ou alta combustível" />
          <Slider label="Multiplicador da perna residual"
                  value={params.mult_pess} min={1.0} max={3.0} step={0.1}
                  onChange={set('mult_pess')}
                  format={(v) => `${fmtNum(v, 1)}x`}
                  hint="Cenário pessimista: terminais limitados" />
        </>
      }
...
function CorredorBlock({ label, number, title, code, route, length, teaser, keyMetrics, sliders, chart, bottomNote, premissas }) {
  return (
    <section style={{ padding: '88px 0', borderTop: `1px solid ${C.rule}` }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 32px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 32, marginBottom: 32, flexWrap: 'wrap' }}>
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 20 }}>
              <Number size="xl" color={C.navy}>{number}</Number>
              <div>
                <h2 style={{
                  fontFamily: fontStack, fontSize: 36, fontWeight: 800, color: C.navy,
        ...
        <p style={{
          fontFamily: fontSerif, fontSize: 22, lineHeight: 1.5, color: C.text,
          maxWidth: 820, marginBottom: 56, fontWeight: 400,
        }}>{teaser}</p>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 64 }}>
          {keyMetrics.map((m, i) => <KeyMetric key={i} {...m} />)}
        </div>
          ...
          <div>
            ...
            <div style={{ background: C.paper, border: `1px solid ${C.rule}`, padding: '24px 16px 12px' }}>
              ...
              <div style={{ display: 'flex', gap: 18, justifyContent: 'center', marginTop: 12, flexWrap: 'wrap' }}>
                {[['Base', C.navy], ['Estressado', C.alert], ['Pessimista', C.green]].map(([l, c]) => (
                  <div key={l} style={{ display: 'flex', alignItems: 'center', gap: 8, fontFamily: fontStack, fontSize: 11, color: C.muted, fontWeight: 500 }}>
                    <span style={{ width: 12, height: 12, background: c, display: 'inline-block' }} />{l}
                  </div>
                ))}
              </div>
            </div>
            <div style={{ fontFamily: fontStack, fontSize: 12, color: C.muted, marginTop: 14, fontStyle: 'italic', lineHeight: 1.6 }}>
              {bottomNote}
            </div>
          </div>
        </div>
...
export default function SobrecustoPage() {
  useEffect(() => {
    ...
    document.title = 'O custo silencioso da rodovia | Observatório IBI';
    ...
    metaDesc.setAttribute('content', 'Análise quantitativa do sobrecusto rodoviário nos corredores Ferrogrão (EF-170) e EF-118. Quanto o Brasil paga por ano em frete evitável.');

    return () => {
      try { document.head.removeChild(link); } catch (e) {}
      document.title = prevTitle;
      if (created) {
        try { document.head.removeChild(metaDesc); } catch (e) {}
      } else {
  ...
  return (
    <div style={{ background: C.paper, color: C.text, fontFamily: 'inherit', minHeight: '100vh' }}>
      {/* HERO */}
      <section style={{
        background: `linear-gradient(180deg, ${C.navy} 0%, ${C.navyDeep} 100%)`,
        padding: '96px 32px 112px', position: 'relative', overflow: 'hidden',
      }}>
        <div style={{
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative' }}>
          ...
          <h1 style={{
            fontFamily: fontStack, fontSize: 'clamp(36px, 5vw, 64px)', fontWeight: 800,
            color: C.cream, margin: '0 0 24px', letterSpacing: '-0.025em', lineHeight: 1.05, maxWidth: 980,
          }}>
            O custo silencioso da rodovia
          </h1>
          <p style={{
            fontFamily: fontSerif, fontSize: 'clamp(18px, 2vw, 24px)', lineHeight: 1.5,
            color: '#B8C2DC', margin: '0 0 64px', maxWidth: 760, fontWeight: 400,
          }}>
          ...
          <div style={{
            marginTop: 64, paddingTop: 24, borderTop: `1px solid #2A3868`,
            display: 'flex', gap: 56, flexWrap: 'wrap', alignItems: 'flex-start',
          }}>
      ...
      {/* FOOTER */}
      <footer style={{ background: C.navyDeep, color: '#9CA3D0', padding: '64px 32px 40px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr', gap: 48 }}>
            <div>
              <div style={{ fontFamily: fontStack, fontSize: 11, color: C.green, letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 700, marginBottom: 14 }}>Sobre o Observatório</div>
              <p style={{ fontFamily: fontStack, fontSize: 13, lineHeight: 1.65, margin: 0, color: '#B8C2DC' }}>
                O Observatório IBI é o braço de inteligência de dados do Instituto Brasileiro de Infraestrutura, vinculado à Frente Parlamentar Mista de Portos e Aeroportos. Produz análises técnicas independentes sobre o setor logístico brasileiro.
              </p>
            </div>
            <div>
              <div style={{ fontFamily: fontStack, fontSize: 11, color: C.green, letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 700, marginBottom: 14 }}>Fontes</div>
              <ul style={{ fontFamily: fontStack, fontSize: 12, lineHeight: 1.85, margin: 0, padding: 0, listStyle: 'none', color: '#B8C2DC' }}>
                <li>Min. Transportes (dez/2025)</li>
                <li>Alvarez & Marsal / Infra S.A.</li>
                <li>Caderno PPI / ANTT</li>
                <li>ESALQ-Log · Sifreca · ILOS</li>
                <li>Findes/Coinfra (NT 001-2025)</li>
                <li>Granel Inteligência de Mercado</li>
              </ul>
            </div>
            <div>
              <div style={{ fontFamily: fontStack, fontSize: 11, color: C.green, letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 700, marginBottom: 14 }}>Metodologia detalhada</div>
              <p style={{ fontFamily: fontStack, fontSize: 12, lineHeight: 1.65, margin: '0 0 14px', color: '#B8C2DC' }}>
                A planilha-modelo com todas as premissas, fórmulas e cenários pode ser solicitada institucionalmente.
              </p>
              <a href="#" style={{
                display: 'inline-flex', alignItems: 'center', gap: 8,
                color: C.green, fontFamily: fontStack, fontSize: 12, fontWeight: 600,
                textDecoration: 'none', borderBottom: `1px solid ${C.green}`, paddingBottom: 2,
              }}>
                Solicitar metodologia <ArrowRight size={14} />
              </a>
            </div>
          </div>
          <div style={{ marginTop: 56, paddingTop: 28, borderTop: `1px solid #1E2D5C`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
            <span style={{ fontFamily: fontStack, fontSize: 11, color: '#6B7280', letterSpacing: '0.05em' }}>
              © {new Date().getFullYear()} Instituto Brasileiro de Infraestrutura · Todos os direitos reservados
            </span>
            <span style={{ fontFamily: fontStack, fontSize: 11, color: '#6B7280', letterSpacing: '0.05em' }}>
